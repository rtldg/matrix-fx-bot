...
//   from https://github.com/matrix-org/matrix-rust-sdk/blob/main/examples/emoji_verification/src/main.rs
//   then edited a little
...
use matrix_sdk::stream::StreamExt as _;
...
async fn print_devices(user_id: &UserId, client: &Client) {
	...
	for device in client.encryption().get_user_devices(user_id).await.unwrap().devices() {
		if device.device_id() == client.device_id().expect("We should be logged in now and know our device id") {
			continue;
		}
